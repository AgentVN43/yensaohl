...
export default function YenTinhChe() {
  const [products, setProducts] = useState([]);
  const { category } = useInit();
  console.log(category);
  const title = _.get(category, '[5].name', null);

  const user = "kenvin146";
  const password = "vIbJjq17S6hpB7Q83HShjd5A";
  const productAPI = `https://yensaohoale.annk.info/wp-json/wc/v3/products?category=17`;
